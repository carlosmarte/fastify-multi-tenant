...
describe('Server Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const mockFS = MockFactories.createMockFS();
  ...
  describe('Server Initialization', () => {
    ...
    test('should validate configuration and fail on errors', async () => {
      ...
      await expect(server.start()).rejects.toThrow('Configuration errors: server.port must be a number between 1 and 65535');
    });
  });
  ...
  describe('Entity Loading Integration', () => {
    ...
    test('should skip inactive entities', async () => {
      ...
      // Mock fast-glob to return config files for each entity
      const fastGlob = await import('fast-glob');
      const originalFastGlob = fastGlob.default;
      fastGlob.default.mockImplementation((pattern, options) => {
        if (pattern === 'config.{json,js,mjs}') {
          if (options?.cwd && (options.cwd.includes('active-tenant') || options.cwd.includes('inactive-tenant'))) {
            return Promise.resolve([path.join(options.cwd, 'config.json')]);
          }
        }
    ...
    test('should handle entity loading errors gracefully', async () => {
      // Define entity configuration
      const entityConfig = {
        entities: {
          definitions: {
      ...
      // Mock fast-glob for config file discovery
      const fastGlob = await import('fast-glob');
      fastGlob.default.mockImplementation((pattern, options) => {
        if (pattern === 'config.{json,js,mjs}') {
          if (options?.cwd && (options.cwd.includes('good-tenant') || options.cwd.includes('bad-tenant'))) {
      ...
      const stats = server.dependencies.entityManager.getStats();
      expect(stats.total).toBe(2); // Both tenants loaded (graceful error handling with defaults)
      expect(stats.history.loaded).toBe(2); // Both successfully processed
    });
  ...
  describe('Global Routes Loading', () => {
    test('should load global routes from routes directory', async () => {
      ...
      expect(server.app.register).toHaveBeenCalledWith(apiRoutes, expect.objectContaining({
        namespace: '/Global/Routes/api'
      }));
      expect(server.app.register).toHaveBeenCalledWith(healthRoutes, expect.objectContaining({
        namespace: '/Global/Routes/health'
      }));
    });
    ...
    test('should continue loading when individual routes fail', async () => {
      const { glob } = await import('glob');
      ...
      const failingRoute = vi.fn().mockRejectedValue(new Error('Route failed'));
      ...
      expect(server.app.register).toHaveBeenCalledWith(workingRoute, expect.any(Object));
  ...
  describe('Error Handling Integration', () => {
    test('should handle configuration validation errors', async () => {
      ...
      await expect(server.start()).rejects.toThrow('Configuration errors');
    });
